...
public:
	...
	/**
	 * Send a USSD to the provided number.
	 */
	bool sendUssd(const char* pin, char* response, size_t responseSize);
	 ...
	 * Extract the specified field from the GPS parsed sequence as a float.
	 */
	bool getGpsField(const char* response, SIM808_GPS_FIELD field, double* result);
	/**
